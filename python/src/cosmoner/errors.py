...
class WebhookSignatureError(CosmonerError):
    """A received webhook could not be verified against its signing secret.

    Unlike the rest of the hierarchy this never comes from an API response —
    it is raised locally while checking an inbound request, so it carries no
    meaningful HTTP status.
    """

    def __init__(self, message: str) -> None:
        """Records the reason verification failed under a fixed error code."""
        super().__init__(0, "WEBHOOK_SIGNATURE_INVALID", message)
